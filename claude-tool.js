...
const credential = Credential.create("Anthropic", "Enter your Anthropic API key");
...
let f = () => {
    ...
    p.addLabel("thinkingLabel", "Uses Claude Opus with extended thinking (slower, smarter)");
    ...
    try {
        ...
        answer = textBlocks.map(b => b.text).join("").trim();

        // Log thinking usage if in thinking mode
        if (thinkingMode) {
            const thinkingBlocks = jsonResponse.content.filter(b => b.type === "thinking");
            const thinkingChars = thinkingBlocks.reduce((sum, b) => sum + (b.thinking?.length || 0), 0);
            if (thinkingChars > 0) {
